...
2. Install the package by calling `colcon build --symlink-install --packages-select robot_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.b-robotized.com) than instead of the previous three steps, use `cb robot_description` command.

Now, launch description test:
```
ros2 launch robot_description view_robot_x.launch.xml
```
or
```
ros2 launch robot_description view_robot_x.launch.py
```
