...
def main():
    ...
    while True:
        ...
        for idx, x in enumerate(response):
            # 26 bytes must remain after idx to read full packet
            if x == 19 and idx <= len(response) - 26:
                packet = response[idx+3:idx+29]
...
4 6 100 27 Wall Signal 2 0 - 1023
...
100 101 52 Ir Opcode Left 1 0 - 255
                ...
                print(
                    f"\rT: {t} Bumps L:{bump_left} R:{bump_right} | "
                    ...
                    f"V:{voltage}mV I:{current}mA T:{temperature}C Ch:{charge}mAh Cap:{capacity}mAh",
                    end="", flush=True
                )
            else:
                continue
