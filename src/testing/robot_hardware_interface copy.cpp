...
namespace robot_hardware_interface
{
      ...
      const rclcpp_lifecycle::State & /*previous_state*/)
  {
    ...
    // Reset Roomba, send 7
    uint8_t reset_cmd = 7;
    if (::write(serial_fd_, &reset_cmd, 1) != 1)
    {
      RCLCPP_ERROR(logger_, "Failed to send RESET command (7)");
    }

    sleep(5); // wait for Roomba to reset
    ...
    sleep(1); // give Roomba time to respond
    ...
    */
    ...
    // --- Write the command safely ---
    size_t total_written = 0;
    while (total_written < sizeof(sensor_cmd)) {
        ssize_t n = ::write(serial_fd_, sensor_cmd + total_written, sizeof(sensor_cmd) - total_written);
        if (n < 0) {
            RCLCPP_ERROR(logger_, "Failed to write encoder request");
        }
    ...
    size_t total_read = 0;
    while (total_read < sizeof(response_2)) {
        ssize_t n = ::read(serial_fd_, response_2 + total_read, sizeof(response_2) - total_read);
        if (n < 0) {
            RCLCPP_ERROR(logger_, "Failed to read encoder counts");
        }
        total_read += static_cast<size_t>(n);
    }

    // --- Convert bytes to uint16_t ---
    // Assuming the device sends data in **big-endian** order (MSB first)
 ...
 /*
   ...
   */
    ...
    while (total_read < sizeof(response)) {
        ssize_t n = ::read(serial_fd_, response + total_read, sizeof(response) - total_read);
        if (n < 0) {
            RCLCPP_ERROR(logger_, "Failed to read encoder counts");
        }
        total_read += static_cast<size_t>(n);
