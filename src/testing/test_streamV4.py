...
def main():
    ...
    while True:
        time.sleep(0.02) 
        all_data = ser.read(ser.in_waiting)  # read everything in buffer
        # Take only the newest bytes
        response = all_data[-170:]
        t +=1
        ...
        for idx, x in enumerate(response):
            ...
            19(start) 81(number of bytes) 100(packet id) ...data bytes (80)... checksum
            '''
            if x == 19 and idx <= len(response) - 80:
